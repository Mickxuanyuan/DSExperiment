...
/* 	var questionTitleObj = document.getElementById("questionTitle");
  ...
  <body style="margin:0px;height: 760px;width:100%">
	<div class="warpper">
		...
		<form  action="<%=PathUtil.getFullPath("admin/editTeacherAction")%>" method="POST" id="editForm">
		<div class="center_question">
		
			<label >老师工号&nbsp;:</label>
			<input id="teacherId" value="${teacher.userId}" name="teacherId" style="width:173px;height:21px; position: absolute; left:150px;top:0px;"/>
			<label class="edit_question_center_title_l" style="top:50px">老师姓名&nbsp;:</label>
			<input class="edit_question_center_title" style="top:50px" value="${teacher.username }" id="teacherName" name="teacherName"/>
			<label class="edit_question_center_title_l" >电话&nbsp;:</label>
			<input class="edit_question_center_title" id="teacherTel" value="${teacher.telephone}" name="teacherTel"></input>
			<label class="edit_question_center_discription_l" >邮箱&nbsp;:</label>
			<input class="edit_question_center_title" style="top:150px" id="teacherEmail" value="${teacher.email}" name="teacherEmail"/>
			<label class="edit_question_center_discription_l" style="top:200px">任教班级&nbsp;:</label>
			<div class="edit_question_center_title" style="top:200px;overflow: auto;height:200px;">
				<c:forEach items="${majorList}" var="o">
				<% boolean state = false; %>
					<c:forEach items="${teacher.majorIds}" var="q">
								<c:if test="${q == o.id}">
								<%state = true;%>
								</c:if>
								</c:forEach>
					<label><input name="class" type="checkbox" value="${o.id}"
					<%if (state){ %>
								checked="checked"
								<% }%>/>${o.name}</label>
					<br/>
			...
			<input id="id" value="${teacher.id}" name="id" style="visibility: hidden;"/>
