  ...
  <body style="margin:0px;height: 760px;width:100%">
	<div class="warpper">
		...
		<div class="main_center_top">
			<label>问题列表&nbsp;--</label><label class="edit_question_id_l">问题详情&nbsp;--</label><label class="edit_question_id">Q<fmt:formatNumber type="number" pattern="######" minIntegerDigits="6" value="${question.id}"/></label>
		</div>
		<form  action="<%=PathUtil.getFullPath("teacher/submitEditResult")%>" method="POST" id="editForm">
		<div class="center_question">
			<label >章节名称&nbsp;:</label>
			<select class="chapter_select" name="options" disabled="disabled">
			...
			<label class="edit_question_center_title" id="questionTitle">${question.title}</label>
			<label class="edit_question_center_discription_l" >问题描述&nbsp;:</label>
			<textarea class="edit_question_center_ds" id="questionDescription"  disabled="disabled" name="questionDescription">${question.description}</textarea>
			<div class="button_sure" onclick="submitEdit('${question.id}')">编辑</div>
